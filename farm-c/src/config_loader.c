...
static bool parse_world(Document *d, ResolvedConfig *c, char **crop_ids, char **upgrade_ids,
                        char **item_ids) {
    g_file = d->name;
    cJSON *root = d->json;
    if (!array(root, d->name)) return false;
    size_t n = (size_t)cJSON_GetArraySize(root);
    (void)crop_ids; (void)upgrade_ids; (void)item_ids;
 ...
 * cJSON pointer. */
static bool parse_scalar_sections(Document *docs, ResolvedConfig *c) {
    g_file=docs[2].name;
    c->fertilizer = (FertilizerConfig){0}; c->watering=(WateringConfig){0}; c->soil_dynamics=(SoilDynamics){0}; c->plot_regen=(PlotRegen){0}; c->contracts=(ContractsConfig){1.15,.45,3,7,.35,6,5,.25}; c->markets=(MarketsConfig){.65,.75}; c->storage=(StorageConfig){0,100,1}; c->processing_capacity=0;
    cJSON *r=docs[2].json,*v;
...
bool config_load_directory(const char *directory, ResolvedConfig *out, ConfigError *error) {
    ...
    if (!parse_world(&docs[0],out,NULL,NULL,NULL) || !parse_upgrades(&docs[1],out) ||
        !validate_unlocks(&docs[0],out) ||
         !parse_scalar_sections(docs,out) || !parse_soil_dynamics(&docs[4],out) ||
